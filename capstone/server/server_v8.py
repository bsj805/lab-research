    ...
    if thisroom.host == thisclnt.number: # 요청한 사람이 host일 때
        try:
            if sum([clnt.is_ready for clnt in thisroom.clnts.values()]) == len(thisroom.clnts)-1: # all ready
                ...
                thisroom.clntorder=[clnt.num for clnt in thisroom.clnts.values()] # teller 선정을 위함
                thisroom.s.release()
                thisroom.curstage=1
                ###########################################################################
                ...
                rspnJson = {
                    ...
                    'user_num': rqstJson['user_num'],
                    'user_li' : dict([('user_num',str(clnt.number)) for clnt in thisroom.clnts.values()])
                }
                rspnBody = json.dumps(rspnJson)
                rspnHead = '\r\n'.join([
                    f"{rqstVer} 200 OK",
...
def REQUEST_VOTE(rqstJson, rqstVer):
    ...
    try:
        ...
        if error==1:
            ...
            'code': "SUCCESS_SCORE",
            'user_num': rqstJson['user_num'],
            'userList': dict([(c.name, c.score) for c in thisroom.clnts]),
            'userPoint':dict([(c.name, c.score - prevscore[c.name]) for c in thisroom.clnts])
            }
        rspnBody = json.dumps(rspnJson)
        rspnHead = '\r\n'.join([
            f"{rqstVer} 200 OK",
