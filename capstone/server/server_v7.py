    ...
    while True:
        ...
        # POST라면 처리 후 응답
            ...
            if rqstJson['code']=="REQUEST_LOGIN":
                ...
                rspnHead, rspnBody = REQUEST_READY(user_num,rqstVer)
...
def REQUEST_READY(user_num,rqstVer):
    thisclnt = CLNTLIST[user_num]
        ...
        try:
            if sum([clnt.is_ready for clnt in thisroom.clnts]) == len(thisroom.clnts): # all ready
                ...
                rspnJson = {
                    ...
                    'user_num': rqstJson['user_num'],
                    'user_li' : [str(clnt.number) for clnt in ROOMLIST.clnts]
                }
                rspnBody = json.dumps(rspnJson)
                rspnHead = '\r\n'.join([
                    f"{rqstVer} 200 OK",
...
def REQUEST_WHO_IS_TELLER(room_num,rqstVer):
...
def REQUEST_KEYWORD(rqstJson,rqstVer):
    ...
    if myroom!=None:
        ...
        thisroom.s.release()
        while(True):
            if(time.time()-thisroom.timestamp >KEYWORD_WAIT_VALUE+15):
                error=1
                break
            if(thisroom.waitcount==0):
                break
