...
class CARDDATA:
    def __init__(self, cardno,cardkeyword,cardvoteusernum):
        self.cardno=cardno
        self.keyword=cardkeyword
        self.voteusernum=cardvoteusernum
...
class CLIENT: # client class
    def __init__(self, sock, addr):
        ...
        self.similarcnt=None # 정답을 모든사람이 다같이 맞춘경우 answer+1 대신 similarcnt+1
...
for oneclnt in CLNTLIST.values():
    oneclnt.score=0
    oneclnt.totalcnt=0
    oneclnt.answer=0
    oneclnt.similarcnt=0
    oneclnt.totaltellercnt=0
    oneclnt.tellerwin=0
    oneclnt.nontellerwin=0
...
CLNTLIST['11'].score = 15 # 얘가 지금 텔러임
CLNTLIST['12'].score = 15
CLNTLIST['13'].score = 15
CLNTLIST['14'].score = 15
...
def ask():
    ...
    while True:
        ...
        try:
            ...
            print(f"[message] {obj} #{no} not found")
            continue
        ...
        print(f"[message] type <option> <obj> [no]")



def comm(clnt):

    # while True: # socket 재사용 안 함
    rqst = clnt.sock.recv(RCV_SIZE)
    
    if rqst != b'': # client로부터 빈 request가 들어오지 않았을 때 (ex. 탭 닫기)
...
def REQUEST_LOGIN(user_id,user_pw,oneclnt,rqstVer):
    ...
    try:
        ...
        rettotalcnt=result[0]["totalcnt"]
        retanswer=result[0]["answer"]
        retsimimlarcnt=result[0]["similarcnt"]
        rettotaltellercnt=result[0]["totaltellercnt"]
        rettellerwin=result[0]["tellerwin"]
        retnontellerwin=result[0]["nontellerwin"]
        a=str(user_pw)
        ...
        if error:
            ...
            oneclnt.score=int(retscore)
            oneclnt.totalcnt=int(rettotalcnt)
            oneclnt.answer=int(retanswer)
            oneclnt.similarcnt=int(retsimilarcnt)
            oneclnt.totaltellercnt=int(rettotaltellercnt)
            oneclnt.tellerwin=int(rettellerwin)
            oneclnt.nontellerwin=int(retnontellerwin)
...
def REQUEST_VOTE(rqstJson, rqstVer):
    ...
    try:
        ...
        if thisclnt is thisteller:
            thisclnt.totaltellercnt+=1
        thisclnt.totalcnt+=1
        if 0 < tellerSelected < len(thisroom.clnts) - 1: # 모두 틀리거나 모두 맞춘 상황이 아니라면

            if thisclnt is thisteller:
                thisclnt.score += 3 # 텔러는 3점을 얻고
                thisclnt.tellerwin+=1
                mydata=CARDDATA(thisclnt.submitcardnum,thisroom.curkeyword,thisclnt.number)
                thisroom.cardlist.append(mydata)
                ...
                if thisclnt.votedcardnum == thisteller.submitcardnum: # teller의 card를 선택한 사람은 3점을 얻고
                    thisclnt.score += 3
                    thisclnt.answer+=1
                    mydata=CARDDATA(thisclnt.votedcardnum,thisroom.curkeyword,thisclnt.number)
                    thisroom.cardlist.append(mydata)
                ...
                for c in thisroom.clnts.values():
                    if ((c.votedcardnum == thisclnt.submitcardnum) and (c.number != thisclnt.number)):
                        thisclnt.score += 1
                        thisclnt.nontellerwin+=1
                        print(f"{thisclnt.name} gets 1 points by the {c.name}'s choice")
                        mydata=CARDDATA(thisclnt.submitcardnum,thisroom.curkeyword,c.number)
                        thisroom.cardlist.append(mydata)
        else: # 모두 틀리거나 모두 맞춘 상황이라면
            
            if thisclnt is not thisteller: # 텔러가 아닌 나는 2점을 먹음, 내가 텔러면 아무 점수도 못 먹음
                thisclnt.score += 2
                 #정답도 아니고 텔러도아닌데 +2점인경우
                if thisclnt.votedcardnum != thisteller.submitcardnum:
                    thisclnt.similarcnt+=1
                else:
                    answer+=1
                mydata=CARDDATA(thisclnt.votedcardnum,thisroom.curkeyword,thisclnt.number)
                thisroom.cardlist.append(mydata)
        ...
        time.sleep(0.1) # 각자의 점수처리가 다 완료될 때까지 충분히 기다려주기
        # 위의 while thisroom.waitcount 시간보다 같거나 커야 한다
        ...
        if len([c for c in thisroom.clnts.values() if c.score >= WINNINGSCORE ]) > 0: # 누군가 15점을 넘었다면
            rspnJson['code'] = "SUCCESS_SCORE_FIN"
            thisroom.curstage = 0
            thisroom.timestamp = time.time()
            thisroom.l.acquire()
            thisroom.waitcount = (thisroom.waitcount + 1) % len(thisroom.clnts)
            thisroom.l.release()
            if( thisroom.waitcount==1):
                threading.Thread(target=uploadcardDB, args=(thisroom.number)).start()
            #sql문 thread로 못빼나?
            sql_update_score="UPDATE `user` SET `user_score` = \"" +str(thisclnt.score)+ "\", `totalcnt` = \"" + str(thisclnt.totalcnt) +"\", `answer` = \""+str(thisclnt.answer)+"\", `similarcnt` = \""+str(thisclnt.similarcnt)+"\", `totaltellercnt` = \""+str(thisclnt.totaltellercnt)+"\", `tellerwin` = \""+str(thisclnt.tellerwin) +"\", `nontellerwin` = \""+str(thisclnt.nontellerwin) +"\" WHERE `usernum`=\""+str(thisclnt.number)+"\";"
            try:
...
def uploadcardDB(roomno): #room의 cardlist 보내주면
    thisroom=ROOMLIST[str(roomno)]
    #print("방번호:"+roomno)
    for carddb in thisroom.cardlist:
        cardno=carddb.cardno
        cardkeyword=carddb.keyword
        usernum=carddb.voteusernum
        sql_request_card="INSERT INTO `card` (cardno,cardkeyword,usernum) VALUES (\""+str(cardno)+"\" , \""+str(cardkeyword)+"\" , \""+str(usernum)+"\");"
        try:
            cursor.execute(sql_request_card)
            main_db.commit()
            result=cursor.fetchall()
            print(result)
        except Exception as e:
