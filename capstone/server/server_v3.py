...
class ROOM: # room class
    ...
    def __init__(self, host, name, maxclnt):
        ...
        self.curstage = 0
            # 0:대기실 타이머, 1:텔러가 키워드 선정중, 2: 키워드를 보고서 카드뽑는중, 4: 게임끝
    ...
    while True:
        ...
        # POST라면 처리 후 응답
            ...
            if rqstJson['code']=="REQUEST_LOGIN":
                ...
                rspnHead, rspnBody = REQUEST_LOGIN(usid, uspw,rqstVer)
...
def REQUEST_LOGIN(user_id,user_pw,rqstVer):
    error=False
    retusernum=0
    retid=0
    retscore=0
...
def REQUEST_ROOM_JOIN(rqstJson,rqstVer):
    ...
    error=0
    if(len(thisroom.clnts)>= thisroom.maxclnt ):
        error=1
    else:
...
def REQUEST_TIMER( rqstJson,rqstVer):
    ...
    if(CLNTLIST[usertarget].roomno==None):#강퇴당했네
