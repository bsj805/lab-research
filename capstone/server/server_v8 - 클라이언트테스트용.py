...
CLNTLIST = dict() # {user_num: CLIENT}
ROOMLIST = dict() # {number: ROOM}
    ...
    while True:
        ...
        # POST라면 처리 후 응답
            ...
            if rqstJson['code']=="REQUEST_LOGIN":
                ...
                group_data["code"]="SUCCESS_START"
                group_data["user_num"]=rqstJson['user_num']
                group_data["user_name"]="abcde"
                group_data["user_li"]=[{"user_num":"11"},{"user_num":"12"},{"user_num":"13"},{"user_num":"14"}]
                ...
                group_data["user_num"]=rqstJson['user_num']
                group_data["room_no"]="1"
                group_data["card_li"]=[{"card_no":"15"},{"card_no":"20"},{"card_no":"3"},{"card_no":"7"},{"card_no":"6"},{"card_no":"1"}]
                ...
                group_data=OrderedDict()
                group_data["code"]="SUCCESS_CARD_ONE"
                group_data["user_num"]=rqstJson['user_num']
                group_data["card_no"]="-1"
                msgstr=str(json.dumps(group_data,ensure_ascii=False,indent="\t"))
                rspnHead = '\r\n'.join([
                        f"{rqstVer} 200 OK",
                        f"Server: {SERVER_NAME}",
                ...
                group_data=OrderedDict()
                group_data["code"]="SUCCESS_WHO_IS_TELLER"
                group_data["user_num"]=rqstJson['user_num']
                group_data["teller_num"]="11"
                ...
                group_data=OrderedDict()
                group_data["code"]="SUCCESS_SCORE"
                group_data["user_li"]=[{"abcde":"3"},{"guest13":"3"},{"guest14":"4"},{"guest12":"0"}]
                group_data["user_Point"]=[{"abcde":"3"},{"guest13":"3"},{"guest14":"4"},{"guest12":"0"}]
                group_data["answer"]="6"      
                msgstr=str(json.dumps(group_data,ensure_ascii=False,indent="\t"))
                rspnHead = '\r\n'.join([
                        f"{rqstVer} 200 OK",
...
def REQUEST_READY(rqstJson, rqstVer):
    user_num = rqstJson['user_num']
    thisclnt = CLNTLIST[user_num]
    ...
    if thisclnt.is_ready == 2: # 게임 중일 때는 무시
        ...
        return rspnHead, rspnBody
    CLNTLIST[thisclnt].readytime=time.time()
    thisroom.s.acquire()
        ...
        try:
            if sum([clnt.is_ready for clnt in thisroom.clnts]) == len(thisroom.clnts): # all ready
                ...
                rspnJson = {
                    ...
                    'user_num': rqstJson['user_num'],
                    'user_li' : [str(clnt.number) for clnt in thisroom.clnts]
                }
                rspnBody = json.dumps(rspnJson)
                rspnHead = '\r\n'.join([
                    f"{rqstVer} 200 OK",
...
def REQUEST_KEYWORD(rqstJson,rqstVer):
    ...
    myroom=None
    error=0
    if roomfind in ROOMLIST.values():
        if CLNTLIST[usertarget].roomno==roomfind:
            error=0
            if CLNTLIST[usertarget] in ROOMLIST[roomfind].clnts:
...
def REQUEST_VOTE(rqstJson, rqstVer):
    ...
    try:
        thisroom.s.acquire()
        thisroom.waitcount = (thisroom.waitcount + 1) % len(thisroom.clnts)
        thisclnt.submitcardnum = rqstJson['vote_num']
        prevscore[thisclnt.name] = thisclnt.score
        thisroom.s.release()

        while len(thisroom.votes) != len(thisroom.clnts):
        ...
        tellerSelected = len([c for c in thisroom.clnts if c.submitcardnum == thisteller.submitcardnum])

        if 0 < tellerSelected < len(thisroom.clnts) - 1: # 모두 틀리거나 모두 맞춘 상황이 아니라면

            thisteller.score += 3 # 텔러는 3점을 얻고

            for clnt in thisroom.clnts:
                if clnt is not thisteller:

                    if clnt.submitcardnum == thisteller.submitcardnum: # teller의 card를 선택한 사람은 3점을 얻고
                        clnt.score += 3
                    else: # teller가 아닌 사람의 card를 선택했다면, 선택 당한 그 사람이 점수를 먹음
                        thatclnt = [c for c in thisroom.clnts if c.submitcardnum == clnt.submitcardnum][0]
                        thatclnt.score += 1
            
        else: # 모두 틀리거나 모두 맞춘 상황이라면
            for clnt in thisroom.clnts:
                if clnt is not thisteller:
                    clnt.score += 2 # 텔러를 제외한 모두가 2점씩 먹음
        if error==1:
            rspnJson = {
            'code': "FAILED_SCORE",
            'user_num': rqstJson['user_num'],
            ...
            'code': "SUCCESS_SCORE",
            'user_num': rqstJson['user_num'],
            'userList': [(c.name, c.score) for c in thisroom.clnts],
            'userPoint':[(c.name, c.score - prevscore[c.name]) for c in thisroom.clnts]
            }
        rspnBody = json.dumps(rspnJson)
        rspnHead = '\r\n'.join([
            f"{rqstVer} 200 OK",
