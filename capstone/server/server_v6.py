...
def REQUEST_ROOM_CREATE(user_num, room_name, room_user):
    try:
        ...
        rspnJson = {
            ...
            'user_num': newRoom.clnts[0],
            'room_name': newRoom.name,
            'room_no': newRoom.number,
            'room_user': newRoom.maxclnt,
            'user_li': [
                {
                    'user_name': "TBD", # member.name
                    'user_num': "TBD", # member.number,
                    'user_score': "TBD", # member.score,
                    'is_ready': "TBD", # member.is_ready
                } for member in newRoom.clnts
            ]
        }
...
def REQUEST_KEYWORD(rqstJson,rqstVer):
    ...
    if error==0:
        usertarget==myroom.curteller:#usertarget이 텔러면:
